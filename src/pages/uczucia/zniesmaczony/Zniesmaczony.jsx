...
export default function Zniesmaczony() {
  const [title, setTitle] = useState("Zniesmaczony");
  
  const [opis, setOpis] = useState("");
  const { user } = useContext(Context);
  let history = useHistory();
 
  const [ratingZniesmaczony, setRatingZniesmaczony] = useState(2);
...
const buttonRef = createRef()
  ...
  const settings = {
    ...
    max: 8,
    step: 1,
    onChange: ratingZniesmaczony => {
      setRatingZniesmaczony(ratingZniesmaczony);
    }
  };
      ...
      <Grid.Column>
          <Segment textAlign='center'>
           ...
           />
            ...
            <Segment   >
            <span style={{marginTop:"7px", color:"darkgray"}} className="etykieta">Podaj natężenie uczucia</span>
           <Label color='red' floating>
            {ratingZniesmaczony} 
           </Label>
           <br/>
           <Slider value={ratingZniesmaczony} color="red" settings={settings} /> 
           </Segment>
          ...
          <br/>
          <Button className='button-550' size="large"   color='grey'  role="button" loading={isLoading} disabled={isLoading1}  onClick={handleClick}>
