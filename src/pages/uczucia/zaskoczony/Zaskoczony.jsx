...
export default function Zaskoczony() {
  const [title, setTitle] = useState("Zaskoczony");
  
  const [opis, setOpis] = useState("");
  const { user } = useContext(Context);
  let history = useHistory();
 
  const [ratingZaskoczony, setRatingZaskoczony] = useState(2);
...
const buttonRef = createRef()
  ...
  const settings = {
    ...
    max: 8,
    step: 1,
    onChange: ratingZaskoczony => {
      setRatingZaskoczony(ratingZaskoczony);
    }
  };
    ...
    <Grid.Row stretched>
      <Grid.Column>
          <Segment textAlign='center'>
           ...
           <br/>
           <Slider value={ratingZaskoczony} color="red" settings={settings} /> 
           </Segment>
          ...
          <br/>
          <Button className='button-59' size="large"   color='purple'  role="button" loading={isLoading} disabled={isLoading1}  onClick={handleClick}>
