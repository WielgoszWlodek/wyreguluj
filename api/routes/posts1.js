...
router.post("/", async (req, res) => {
  const newPost = new Post1(req.body);
  try {
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (post.username === req.body.username) {
      try {
        const updatedPost = await Post1.findByIdAndUpdate(
          req.params.id,
          {
            $set: req.body,
          },
...
//https://dev.to/hakimraissi/pagination-with-express-and-mongoose-pnh
  try {
    let posts;
    if (username) {
      posts = await Post1.find({ username : username }).limit(limitValue)
      .skip(skipValue).sort({ createdAt: -1 });
    }  else {
      posts = await Post1.find();
    }
    res.status(200).json(posts);
  } catch (err) {
    res.status(500).json(err);
